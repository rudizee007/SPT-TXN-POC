...
docname: draft-coetzee-oauth-spt-txn-tokens-02
category: exp
ipr: trust200902
area: Security
...
a byproduct of enforcement. This revision adds normative specifications for
...
Version -01 specified the base token format, the offline
verification model, and jurisdictional token-bound access control. This
revision restates that base model self-containedly ({{token-model}}) and adds
normative Sections {{intent-binding}} (intent binding),
...
implementation. It also hardens {{attested-issuance}} to make the
audience and expiry checks on the presented attestation mandatory, to require
an issuer-side bound on delegated depth, and to clamp a token's lifetime to its
attestation. The issuer's requested-versus-permitted scope intersection is
enforced at mint and re-checked independently by the enforcement point
({{delegation}}). The changes reflect an adversarial "find the
bypass" review of the reference bridges and carry no change to the token
format.
